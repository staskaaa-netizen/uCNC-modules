...
#ifdef G33_FEEDBACK_LOOP_USE_HW_COUNTER
				DBGLOG("G33 defer start hw=%ld idx=%ld empty=1", hw_now, index);
#else
				DBGLOG("G33 defer start idx=%ld empty=1", index);
#endif
			}
#endif
			break;
...
#ifdef ENABLE_G33_DEBUG
#ifdef G33_FEEDBACK_LOOP_USE_HW_COUNTER
		DBGLOG("G33 start hw=%ld idx=%ld empty=%u",
		           hw_now, index, (unsigned)itp_is_empty());
#else
		DBGLOG("G33 start idx=%ld empty=%u",
...
bool g33_exec(void *args)
{
	...
	if (ptr->cmd->group_extended == EXTENDED_MOTION_GCODE(33))
	{
...
#ifdef ENABLE_G33_DEBUG
			DBGLOG("G33 measuring hw rpm enc_res=%lu min_counts=%lu",
...
#ifdef ENABLE_G33_DEBUG
			DBGLOG("G33 hw rpm sample dt=%lu counts=%ld rpm=%f enc_rpm=%u idx_count=%ld",
...
#ifdef ENABLE_G33_DEBUG
		DBGLOG("G33 waiting for index sample idx_rev=%lu enc_res=%lu",
		...
		for (;;)
		{
			...
			{
#ifdef ENABLE_G33_DEBUG
				DBGLOG("G33 index wait timeout t=%lu last=%lu count=%ld",
			...
			if ((mcu_millis() - index_wait_last_print) > 1000U)
			{
				...
				DBGLOG("G33 still waiting index t=%lu last=%lu count=%ld",
				           (unsigned long)delta_t, (unsigned long)t, (long)spindle_index_counter);
			}
#endif
			cnc_dotasks();
...
#ifdef ENABLE_G33_DEBUG
		DBGLOG("G33 init total_steps=%lu total_revs=%f steps_rev=%f enc_res=%lu idx_rev=%lu spi=%lu q16=%ld rpm_const=%f feed=%f",
...
#ifdef ENABLE_G33_DEBUG
		DBGLOG("G33 armed empty=%u planner=%u start_idx=%ld accel_revs=%f g33_accel=%f",
	...
	if ((synched_motion_status & SYNC_UPDATED))
	{
...
#ifdef ENABLE_G33_DEBUG
		{
			...
			if (debug_div >= G33_DEBUG_EVERY_N)
			{
				debug_div = 0;
#ifdef G33_FEEDBACK_LOOP_USE_HW_COUNTER
				/*DBGLOG("G33 hw=%ld org=%ld sorg=%ld dt=%lu rpm=%f base=%f corr=%f q16=%ld off=%ld exp=%ld real=%ld err=%ld",
				...
				DBGLOG("G33 exp=%ld real=%ld err=%ld",
			             expected_position, index_step_counter, error);
#else
				DBGLOG("G33 idx=%ld dt=%lu rpm=%f base=%f corr=%f exp=%ld real=%ld err=%ld",
		...
		if (elapsed > 1000)
		{
			...
			if (delta_t)
			{
...
#ifdef G33_FEEDBACK_LOOP_USE_HW_COUNTER
				{
					...
					DBGLOG("G33 wait st=%u idxrpm=%f hwrpm=%f hw=%ld empty=%u",
					           (unsigned)synched_motion_status, index_rpm, hw_rpm,
					           hw_now, (unsigned)itp_is_empty());
				}
#else
				DBGLOG("G33 wait st=%u rpm=%f empty=%u",
